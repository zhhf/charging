...
from charging.common import constants
from charging.common import exceptions as exception
...
from charging.openstack.common import jsonutils
...
class Server(object):
    ...
    def start(self, application, port, host='0.0.0.0', workers=0):
        ...
        self._socket = self._get_socket(self._host,
                                        self._port,
                                        backlog=backlog)
        LOG.debug('zhf.host=%s,port=%s,backlog=%s' % (host,port,backlog))
        LOG.debug('zhf.socket=%s' % self._socket)
        if workers < 1:
...
class Request(webob.Request):
    ...
    @property
    def context(self):
        if 'charging.context' not in self.environ:
            self.environ['charging.context'] = context.get_admin_context()
        return self.environ['charging.context']
...
class Router(object):
    ...
    def _dispatch(req):
        ...
        LOG.debug('zhf.match=%s' % match)
        #LOG.debug('zhf.environ=%s' % req.environ['wsgiorg.routing_args'])
        if not match:
