...
from charging.api.v1 import resource as wsgi_resource
#from neutron.common import constants as const
from charging.common import exceptions
...
class ControllerNew(object):
    def __init__(self, plugin=None, collection=None, resource=None, attr_info=None,
                 allow_bulk=False, member_actions=None, parent=None,
                 allow_pagination=False, allow_sorting=False):
        return
    ...
    def create(self, request, id, **kwargs):
        ...
        if resource not in ['flavor', 'tenant']:
            msg = _('The resource could not be found.')
            raise webob.exc.HTTPNotFound(msg)
        body = kwargs.get('body', None)
        if not body:
            webob.exc.HTTPBadRequest(_("Resource body required"))
        
        if resource == 'tenant':
            model_ref = models_v2.Tenant()
            total_money = body.get('total_money', 0)
            value = {'tenant_id': id,
                     'name': body.get('name', None),
                     'domain_id': body.get('domain', 'default'),
                     'total_money': total_money,
                     'remainder_money': body.get('remainder_money', total_money), }
        else:
            model_ref = models_v2.Flavor()
            value = {'flavorid': id,
                     'name':  body.get('name', None),
                     'unit_price': body.get('unit_price', 0), }
        model_ref.update(value)
        model_ref.save()
    ...
    def show(self, request, id, **kwargs):
        ...
        with request.context.session.begin(subtransactions=True):
            try:
                if resource == 'tenant':
                    query = request.context.session.query(models_v2.Tenant)
                    tenant = query.filter(models_v2.Tenant.tenant_id==id, models_v2.Tenant.deleted==0).one()
                    res = {'tenant_id':  tenant['tenant_id'], 'name': tenant['name'],
                            "domain_id": tenant["domain_id"], 
                            "total_money": tenant["total_money"], 
                            'remainder_money': tenant['remainder_money']}
                else:
                    query = request.context.session.query(models_v2.Flavor)
                    flavor = query.filter(models_v2.Flavor.flavorid==id, models_v2.Flavor.deleted==0).one()
                    res = {'flavorid':  flavor['flavorid'], 'name': flavor['name'],
                           'unit_price': flavor["unit_price"]}
                return res
            except exc.NoResultFound:
                #raise exceptions.TenantNotFound(tenant_id=tenant_id)
                msg = _('The resource could not be found.')
                raise webob.exc.HTTPNotFound(msg)

    def list(self, request, **kwargs):
        resource = kwargs.get('resource', None)
        #LOG.debug('zhf.resource_list=%s' % resource)
        if resource not in ['flavor', 'tenant']:
            msg = _('The resource could not be found.')
            raise webob.exc.HTTPNotFound(msg)
        ...
        with request.context.session.begin(subtransactions=True):
            try:
                if resource == 'tenant':
                    query = request.context.session.query(models_v2.Tenant)
                    tenants = query.filter(models_v2.Tenant.deleted==0)
                    res = [{'tenant_id': tenant['tenant_id'],
                            'name': tenant['name'],
                            'remainder_money': tenant['remainder_money']} for tenant in tenants]
                else:
                    query = request.context.session.query(models_v2.Flavor)
                    flavors = query.filter(models_v2.Flavor.deleted==0)
                    res = [{'flavorid': flavor['flavorid'],
                            'name': flavor['name'],
                            'unit_price': flavor['unit_price']} for flavor in flavors]
                return res
            except exc.NoResultFound:
                #raise exceptions.TenantNotFound(tenant_id=tenant_id)
                msg = _('The resource could not be found.')
                #LOG.debug('zhf.resource_list=in except')
                raise webob.exc.HTTPNotFound(msg)

    def delete(self, request, id, **kwargs):
        #LOG.debug('zhf. in delete')
        resource = kwargs.get('resource', None)
        if resource not in ['flavor', 'tenant']:
            msg = _('The resource could not be found.')
            raise webob.exc.HTTPNotFound(msg)
        ...
        with request.context.session.begin(subtransactions=True):
            if resource == 'tenant':
                query = (request.context.session.query(models_v2.Tenant).
                    enable_eagerloads(False).filter_by(tenant_id=id))
            else:
                query = (request.context.session.query(models_v2.Flavor).
                    enable_eagerloads(False).filter_by(flavorid=id))

            #table = query.with_lockmode('update').one()
            #request.context.session.delete(table)
            count = query.soft_delete()
            if count == 0:
                raise exceptions.BadRequest(resource='body', msg='deleting is error!')
    ...
    def update(self, request, id, body=None, **kwargs):
        ...
        with request.context.session.begin(subtransactions=True):
            if resource == 'tenant':
                query = request.context.session.query(models_v2.Tenant)
                table = query.filter(models_v2.Tenant.tenant_id==id, models_v2.Tenant.deleted==0).one()
                recharging = payload.pop('recharging_money')
                payload['total_money'] = recharging + table.total_money
                payload['remainder_money'] = recharging + table.remainder_money
            else:
                query = request.context.session.query(models_v2.Flavor)
                table = query.filter(models_v2.Flavor.flavorid==id, models_v2.Flavor.deleted==0)
                #LOG.debug('zhf.update count=%s' % table.count())
                if table.count() == 0:
                    model_ref = models_v2.Flavor()
                    value = {'flavorid': id,
                            'name':  body.get('name', None),
...
def create_resource_new(collection=None, resource=None, plugin=None, params=None, 
                    allow_bulk=False, member_actions=None, parent=None, allow_pagination=False,
                    allow_sorting=False):
